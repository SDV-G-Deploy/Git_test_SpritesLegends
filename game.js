const DIFFICULTIES = {
  story: {
    ...
    eventChance: 0.6,
    effectText: "Герой: +16% HP, враги: -12% HP и -10% урон, 3 зелья, мягче аффиксы."
  },
  normal: {
    key: "normal",
    title: "Норма",
    ...
    scoreMult: 1,
    affixChance: 0.46,
    rareRelicChance: 0.18,
    eventChance: 0.68,
    effectText: "Базовый баланс: 2 зелья, стандартные HP/урон врагов, умеренный риск аффиксов."
  },
  heroic: {
    key: "heroic",
    title: "Героик",
    ...
    scoreMult: 1.28,
    affixChance: 0.68,
    rareRelicChance: 0.26,
    eventChance: 0.78,
    effectText: "Герой: -6% HP, враги: +15% HP и +14% урон, высокий шанс аффиксов."
  }
};
...
const RARE_RELICS = [
  ...
  },
  {
    ...
    name: "Сердце бури",
    tier: "S",
    description: "+1 AP каждый ход, но в начале хода герой теряет 5 HP.",
    apply(stateRef) {
      stateRef.mods.apPerTurn += 1;
      stateRef.mods.selfTickDamage += 5;
    }
  },
  {
    id: "void_grimoire",
  ...
  },
  {
    ...
    tier: "S",
    description: "Обычные удары: +48% урона, но входящий урон по герою +20%.",
    apply(stateRef) {
      stateRef.mods.strikeBonus += 0.48;
  ...
  },
  {
    ...
    name: "Сердце титана",
    tier: "S",
    description: "+34 макс. HP и +1 атака, но в 1-й ход боя -1 AP.",
    apply(stateRef) {
      stateRef.hero.maxHp += 34;
      stateRef.hero.hp = Math.min(stateRef.hero.maxHp, stateRef.hero.hp + 34);
      stateRef.mods.attackFlat += 1;
      stateRef.mods.firstTurnAp -= 1;
    }
  },
  {
    ...
    name: "Кости тени",
    tier: "S",
    description: "+22% крит-шанс и +30% очков, но -20 макс. HP.",
    apply(stateRef) {
      stateRef.mods.critChance += 0.22;
...
const MEGA_REWARDS = [
  {
    id: "mega_overdrive",
    name: "Mega: Overdrive Core",
    tier: "M",
    description: "+4 атака и +25% сила способности.",
    apply(stateRef) {
      stateRef.mods.attackFlat += 4;
      stateRef.mods.specialPower += 0.25;
    }
  },
  {
    id: "mega_aegis",
    name: "Mega: Aegis Matrix",
    tier: "M",
    description: "+36 макс. HP и +8 к эффективности защиты.",
    apply(stateRef) {
      stateRef.hero.maxHp += 36;
      stateRef.hero.hp = Math.min(stateRef.hero.maxHp, stateRef.hero.hp + 36);
      stateRef.mods.guardBonus += 8;
    }
  },
  {
    id: "mega_soulforge",
    name: "Mega: Soulforge",
    tier: "M",
    description: "+18% вампиризм и +10% крит-шанс.",
    apply(stateRef) {
      stateRef.mods.lifesteal += 0.18;
      stateRef.mods.critChance += 0.1;
    }
  },
  {
    id: "mega_arsenal",
    name: "Mega: Arsenal Link",
    tier: "M",
    description: "+1 зелье, +18 сила зелий и +1 AP первого хода.",
    apply(stateRef) {
      stateRef.potions += 1;
      stateRef.mods.potionBonus += 18;
      stateRef.mods.firstTurnAp += 1;
    }
  }
];
const MOVE_LIBRARY = {
  ...
  hex_jab: {
    name: "Проклятый укол",
    intentText: "5-7 урона, шанс Ожога 1.",
    type: "attack",
    hits: 1,
    damage: [5, 7],
    onHit(context) {
      if (chance(context.rng, 0.35)) {
        addStatus(context.hero.statuses, "burn", 1);
        log("Гекс-страж накладывает Ожог 1.", "bad");
      }
    }
  },
  rune_sunder: {
    name: "Рунный надлом",
    intentText: "4-6 урона и Vulnerable 1.",
    type: "attack",
    hits: 1,
    damage: [4, 6],
    onHit(context) {
      addStatus(context.hero.statuses, "vulnerable", 1);
      log("Гекс-страж ослабляет защиту героя (Vulnerable 1).", "bad");
    }
  },
  warding_veil: {
    name: "Завеса печатей",
    intentText: "+10 блока и +2 к следующей атаке.",
    type: "buff",
    execute(context) {
      context.enemy.block += 10;
      context.enemy.tempAttack += 2;
      log("Гекс-страж поднимает рунный барьер.", "bad");
    }
  },
  soul_tithe: {
    name: "Дань души",
    intentText: "2x 3-5 урона, лечение 4 при попадании.",
    type: "attack",
    hits: 2,
    damage: [3, 5],
    afterAttack(context) {
      if (context.landedHits <= 0) {
        return;
      }

      const heal = Math.min(4, context.enemy.maxHp - context.enemy.hp);

      if (heal > 0) {
        context.enemy.hp += heal;
        log(`Гекс-страж поглощает силу и лечится на ${heal} HP.`, "bad");
      }
    }
  },
  dragon_flame: {
...
const STATUS_META = {
  bleed: {
    ...
    icon: "BLD",
    tone: "dot",
    description: "4 урона в начале хода, затем -1 стек."
  },
  burn: {
    label: "Ожог",
    icon: "BRN",
    tone: "dot",
    description: "5 урона в начале хода, затем -1 стек."
  },
  vulnerable: {
    label: "Vulnerable",
    icon: "VUL",
    tone: "debuff",
    description: "+30% входящего урона по цели."
  },
  weaken: {
    label: "Weaken",
    icon: "WEK",
    tone: "debuff",
    description: "-25% исходящего урона цели."
  },
  inspire: {
    label: "Inspire",
    icon: "INS",
    tone: "buff",
    description: "+5 к следующему удару (тратится по 1)."
  },
  exposed: {
    label: "Exposed",
    icon: "EXP",
    tone: "buff",
    description: "Цель получает +25% входящего урона."
  },
  poison: {
    label: "Poison",
    icon: "PSN",
...
function createEnemy(templateId, tier, opts = {}) {
  ...
  const enemy = {
    ...
    tier,
    stage: opts.stage || 1,
    stageBattle: opts.stageBattle || 1,
    miniBoss: Boolean(opts.miniBoss),
    stageBoss: Boolean(opts.stageBoss),
    bossNumber: opts.bossNumber || 0
  };

  enemy.hp = enemy.maxHp;

  if (!boss) {
    const affixChance = elite ? Math.min(1, state.difficulty.affixChance + 0.2) : state.difficulty.affixChance;
  ...
  if (boss && opts.bossProfile) {
    const profile = opts.bossProfile;
    enemy.name = profile.name;
    enemy.maxHp = Math.max(60, Math.round(enemy.maxHp * profile.hpMult));
    enemy.hp = enemy.maxHp;
    enemy.damageScale *= profile.dmgMult;
    enemy.flags.phaseScale = profile.phaseScale;
  }

  return enemy;
}

function buildBossProfile(bossNumber) {
  if (bossNumber === 1) {
    return {
      name: "weak little gragon",
      hpMult: 0.85,
      dmgMult: 0.85,
      phaseScale: 0.85
    };
  ...
  const ramp = bossNumber - 3;
  const variants = [
    { name: "elder dragon", hpBase: 1.1, dmgBase: 1.08, phaseBase: 1.08 },
    { name: "void dragon", hpBase: 1.08, dmgBase: 1.11, phaseBase: 1.1 },
    { name: "storm dragon", hpBase: 1.06, dmgBase: 1.13, phaseBase: 1.12 }
  ];
  const picked = pick(variants);

  return {
    name: picked.name,
    hpMult: picked.hpBase + ramp * 0.08,
    dmgMult: picked.dmgBase + ramp * 0.07,
    phaseScale: picked.phaseBase + ramp * 0.06
  };
}
...
function generateEncounters() {
  ...
  const elitePool = ["werewolf", "golem", "hex_warden"];
  let bossCounter = 0;

  for (let stage = 1; stage <= RUN_STRUCTURE.stages; stage += 1) {
    for (let battle = 1; battle <= 3; battle += 1) {
      const tierBase = 1 + (stage - 1);
      const tier = tierBase + (battle >= 3 ? 1 : 0);
      list.push(createEnemy(pick(regularPool), tier, { stage, stageBattle: battle }));
    }

    const miniBossTier = Math.min(5, stage + 2);
    list.push(createEnemy(pick(elitePool), miniBossTier, {
      elite: true,
      stage,
      stageBattle: RUN_STRUCTURE.battlesPerStage,
      miniBoss: true
    }));

    if (stage % RUN_STRUCTURE.bossEveryStages === 0) {
      bossCounter += 1;
      const bossTier = Math.min(6, stage + 2);
      const bossProfile = buildBossProfile(bossCounter);
      list.push(createEnemy("dragon", bossTier, {
        boss: true,
        stage,
        stageBattle: 0,
        stageBoss: true,
        bossNumber: bossCounter,
        bossProfile
      }));
    }
  }
...
function handleDragonPhaseTransition() {
  ...
  if (state.enemy.phase === 1 && ratio <= 0.7) {
    ...
    const phaseScale = state.enemy.flags.phaseScale || 1;
    state.enemy.damageScale *= 1 + 0.08 * phaseScale;
  ...
  if (state.enemy.phase === 2 && ratio <= 0.35) {
    ...
    const phaseScale = state.enemy.flags.phaseScale || 1;
    state.enemy.damageScale *= 1 + 0.11 * phaseScale;
    state.enemy.tempAttack += Math.max(1, Math.round(3 * phaseScale));
...
function performEnemyIntent() {
  ...
  if (move.afterAttack) {
    move.afterAttack({ hero: state.hero, enemy: state.enemy, rng, landedHits });
  }
...
function renderStatuses(container, statuses) {
  ...
  entries.forEach((entry) => {
    ...
    tag.className = `status-tag tone-${entry.tone}`;
    tag.title = entry.description ? `${entry.label}: ${entry.description}` : entry.label;
    tag.innerHTML = `<span class="status-icon">${entry.icon}</span><span class="status-text">${entry.label} x${entry.value}</span>`;
...
function renderRelics() {
  ...
  state.relics.forEach((relic) => {
    ...
    const tier = relic.tier === "M" ? "[M] " : (relic.tier === "S" ? "[S] " : "");
...
function updateBattleUI() {
  ...
  if (state.enemy.stageBoss) {
    ui.encounterLabel.textContent = `Этап ${state.enemy.stage}: Босс`;
  } else {
    ui.encounterLabel.textContent = `Этап ${state.enemy.stage} · Бой ${state.enemy.stageBattle}/${RUN_STRUCTURE.battlesPerStage}`;
  }
...
function beginEncounter() {
  ...
  if (state.intermissionNote) {
    log(state.intermissionNote, "good");
    state.intermissionNote = "";
  }

  if (state.enemy.stageBoss) {
    log(`Этап ${state.enemy.stage}: бой с боссом ${state.enemy.name}.`, "bad");
  } else {
    log(`Этап ${state.enemy.stage}, бой ${state.enemy.stageBattle}/${RUN_STRUCTURE.battlesPerStage}: ${state.enemy.name}.`, "bad");
  }

  if (state.enemy.miniBoss && !state.enemy.stageBoss) {
    log("Мини-босс этапа: бой повышенной угрозы.", "bad");
  }
...
function showBossHub(stageNumber, bossNumber, restHeal) {
  state.currentEvent = "boss_hub";
  ui.eventGrid.innerHTML = "";

  const stageNote = `Босс этапа ${stageNumber} повержен. +${restHeal} HP.`;
  ui.eventTitle.textContent = `Трофеи босса #${bossNumber}`;
  ui.eventText.textContent = "Выбери одно крутое улучшение (mega skill / mega item).";

  const available = MEGA_REWARDS.filter((reward) => !state.relicSet.has(reward.id));
  const choices = available.length > 0 ? sampleUnique(available, Math.min(3, available.length)) : [];

  if (choices.length === 0) {
    const card = document.createElement("button");
    card.type = "button";
    card.className = "event-card";
    card.innerHTML = "<h3>Дальше в путь</h3><p>Все mega-улучшения уже получены.</p>";
    card.addEventListener("click", () => {
      state.intermissionNote = `${stageNote} Новых mega-улучшений не осталось.`;
      beginEncounter();
    });
  ...
  choices.forEach((reward) => {
    ...
    card.type = "button";
    card.className = "event-card";
    card.innerHTML = `<h3>[MEGA] ${reward.name}</h3><p>${reward.description}</p>`;
    card.addEventListener("click", () => {
      obtainRelic(reward, `Трофей босса #${bossNumber}`);
      state.intermissionNote = `${stageNote} Получено mega-улучшение: ${reward.name}.`;
      beginEncounter();
    });
    ui.eventGrid.appendChild(card);
  });
...
function showStageHub(stageNumber, restHeal) {
  state.currentEvent = "stage_hub";
  ui.eventGrid.innerHTML = "";

  const stageNote = `Этап ${stageNumber} завершен. +${restHeal} HP.`;
  const commonCost = 9 + stageNumber;
  const rareCost = 22 + stageNumber * 2;
  const potionCost = 8 + stageNumber;

  ui.eventTitle.textContent = `Железный этап: привал после этапа ${stageNumber}`;
  ui.eventText.textContent = "Большой выбор: обычные перки дешевле, S-tier заметно дороже.";

  function appendHubCard(title, description, action) {
    const card = document.createElement("button");
    card.type = "button";
    card.className = "event-card";
    ...
    card.addEventListener("click", () => {
      ...
      state.intermissionNote = [stageNote, note].filter(Boolean).join(" ");
      beginEncounter();
    });
    ui.eventGrid.appendChild(card);
  }

  appendHubCard("Продолжить путь", "Без дополнительных трат.", () => "Герой продолжает забег.");

  appendHubCard(
    "Купить зелье",
    `Цена: ${potionCost} HP. +1 зелье.`,
    () => {
      if (state.hero.hp <= potionCost + 1) {
        ui.eventText.textContent = "Недостаточно HP для покупки зелья.";
        return null;
      }

      state.hero.hp -= potionCost;
      state.potions += 1;
      return `Куплено зелье: -${potionCost} HP, +1 зелье.`;
    }
  );

  const commonAvailable = RELICS.filter((relic) => !state.relicSet.has(relic.id));
  commonAvailable.forEach((relic) => {
    appendHubCard(
      `Перк: ${relic.name}`,
      `Цена: ${commonCost} HP. ${relic.description}`,
      () => {
        if (state.hero.hp <= commonCost + 1) {
          ui.eventText.textContent = "Недостаточно HP для покупки перка.";
          return null;
        }

        state.hero.hp -= commonCost;
        obtainRelic(relic, "Железный этап");
        return `Куплен перк: -${commonCost} HP.`;
      }
    );
  });

  const rareAvailable = RARE_RELICS.filter((relic) => !state.relicSet.has(relic.id));
  rareAvailable.forEach((relic) => {
    appendHubCard(
      `S-tier: ${relic.name}`,
      `Цена: ${rareCost} HP. ${relic.description}`,
      () => {
        if (state.hero.hp <= rareCost + 1) {
          ui.eventText.textContent = "Недостаточно HP для покупки S-tier перка.";
          return null;
        }

        state.hero.hp -= rareCost;
        obtainRelic(relic, "Железный этап");
        return `Куплен S-tier перк: -${rareCost} HP.`;
      }
    );
  });
...
function showIntermissionEvent(restHeal, options = {}) {
  const {
    forcedEventId = null,
    followWithReward = true,
    stageNote = ""
  } = options;
  const eventId = forcedEventId || pick(["merchant", "altar", "cursed"]);
  ...
  if (eventId === "merchant") {
    ...
    choices.push({
      name: "Купить зелье",
      description: "Платишь 12 HP и получаешь +1 зелье.",
      apply() {
        if (state.hero.hp <= 13) {
          ui.eventText.textContent = "Не хватило здоровья для сделки.";
          return null;
        }
        state.hero.hp -= 12;
        state.potions += 1;
        return "Сделка проведена: -12 HP, +1 зелье.";
      }
    });
    choices.push({
      name: "Заточить оружие",
      description: "Платишь 7 HP и получаешь +2 к атаке навсегда.",
      apply() {
        if (state.hero.hp <= 8) {
          ui.eventText.textContent = "Слишком рискованно: здоровья мало.";
          return null;
        }
        state.hero.hp -= 7;
        state.mods.attackFlat += 2;
        return "Клинок заточен: -7 HP, +2 атака.";
      }
    });
    choices.push({
      name: "Уйти",
  ...
  if (eventId === "altar") {
    ...
    choices.push({
      ...
      description: "-16 HP, но +2 атака и +8% крит-шанс.",
      apply() {
        if (state.hero.hp <= 17) {
          ui.eventText.textContent = "Клятва отвергнута: мало здоровья.";
          return null;
        }
        state.hero.hp -= 16;
        state.mods.attackFlat += 2;
    ...
    choices.push({
      ...
      apply() {
        if (state.potions <= 0) {
          ui.eventText.textContent = "Нет зелий для ритуала.";
          return null;
        }
        state.potions -= 1;
  ...
  choices.forEach((choice) => {
    ...
    card.addEventListener("click", () => {
      ...
      const combinedNote = [stageNote, note].filter(Boolean).join(" ");

      if (followWithReward) {
        buildRewardChoices(restHeal, combinedNote);
      } else {
        state.intermissionNote = combinedNote;
        beginEncounter();
      }
    });
    ui.eventGrid.appendChild(card);
...
function finishEncounter() {
  ...
  if (state.encounterIndex >= state.encounters.length - 1) {
    finishRun(true, "Босс повержен. Этапы зачищены.");
    return;
  }

  state.encounterIndex += 1;

  if (defeatedEnemy.stageBoss) {
    const bossRest = Math.round(state.hero.maxHp * 0.14) + state.mods.postBattleHeal;
    state.hero.hp = Math.min(state.hero.maxHp, state.hero.hp + bossRest);
    showBossHub(defeatedEnemy.stage, defeatedEnemy.bossNumber || 1, bossRest);
    return;
  }

  const stageCompleted = defeatedEnemy.stageBattle === RUN_STRUCTURE.battlesPerStage;

  if (stageCompleted) {
    const restHeal = Math.round(state.hero.maxHp * 0.12) + state.mods.postBattleHeal;
    state.hero.hp = Math.min(state.hero.maxHp, state.hero.hp + restHeal);
    showStageHub(defeatedEnemy.stage, restHeal);
    return;
  }

  const interFightHeal = Math.round(state.hero.maxHp * 0.04);
  state.hero.hp = Math.min(state.hero.maxHp, state.hero.hp + interFightHeal);

  if (chance(rng, state.difficulty.eventChance)) {
    showIntermissionEvent(interFightHeal, {
      followWithReward: false,
      stageNote: `Короткая встреча между боями. +${interFightHeal} HP.`
    });
  } else {
    state.intermissionNote = `Передышка между боями: +${interFightHeal} HP.`;
    beginEncounter();
  }
}
